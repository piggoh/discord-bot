...
class DiscordMonitor:
    def __init__(self):
        ...
        self.source_server = os.getenv('SOURCE_SERVER_NAME', 'cooks')
        self.source_channel = os.getenv('SOURCE_CHANNEL', 'announcement')
        self.dest_server = os.getenv('DEST_SERVER_NAME', '')
        self.dest_channel = os.getenv('DEST_CHANNEL', 'announcement')
    ...
    async def login_to_discord(self):
        ...
        print("⏳ Waiting for login form (or checking if already logged in)...")
        # If already logged in, Discord will take us straight to channels; skip waiting for form
        try:
        ...
        try:
            ...
            for idx, message_element in enumerate(message_elements):
                try:
                    ...
                    if not message_data or message_data['message_id'] in self.processed_messages:
                        continue
                    ...
                    if not ('OCULUS TRADING SIGNAL' in content or 'Ticker :' in content):
                        continue
                    ...
                    try:
                        ...
                        if timestamp_str:
                            # Check for "Today at" or recent time indicators
                            if 'Today at' in timestamp_str or 'Just now' in timestamp_str:
                                is_recent = True
                            elif 'PM' in timestamp_str or 'AM' in timestamp_str:
                                # Check if it's a recent time (within last few minutes)
                                time_part = timestamp_str.split('—')[-1].strip()
                                if any(indicator in time_part for indicator in ['PM', 'AM']):
                                    # For now, assume recent if it has time format
                                    is_recent = True
                    except Exception:
    ...
    async def post_message(self, message_data: Dict[str, Any]):
        """Post a message to destination Discord channel"""
        try:
            # Ensure we’re already on the correct channel page
            if self.dest_channel_url.strip():
                print(f"🌐 Ensuring correct channel: {self.dest_channel_url.strip()}")
                current_url = self.page.url
                if self.dest_channel_url.strip() not in current_url:
                    print(f"🔄 Navigating to: {self.dest_channel_url.strip()}")
                    await self.page.goto(self.dest_channel_url.strip())

            print("🔍 Locating message input box...")
            selectors = [
                '[data-slate-editor="true"]',
                'div[role="textbox"][contenteditable="true"]',
                '[aria-label^="Message #"][data-slate-editor="true"]',
            ]
            ...
            if not message_input:
                print("❌ Could not find any valid message input (check permissions or channel type).")
                return False
            ...
            print(f"⌨️ Typing message: {content[:100]}...")
            await message_input.click()
            await self.page.keyboard.type(content)
            await self.page.keyboard.press("Enter")

            print("✅ Message sent successfully! Waiting briefly for confirmation...")
            await asyncio.sleep(2)

            # Optionally check if message appears
            try:
                recent_messages = await self.page.query_selector_all('[data-list-id="chat-messages"] [id^="chat-messages-"]')
                if recent_messages:
                    last_message = recent_messages[-1]
                    last_content = await last_message.evaluate('el => el.textContent || el.innerText || ""')
                    if content[:50] in last_content:
                        print("✅ Message confirmed visible in chat.")
                        return True
                    else:
                        print(f"⚠️ Last message mismatch: '{last_content[:80]}...'")
                        print("❌ Message may not have been sent.")
                        return False
            except Exception as e:
                print(f"⚠️ Could not verify message was sent: {e}")
